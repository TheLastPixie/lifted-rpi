...
__all__ = [
    ...
    "GraphSet",
    "box_corners",
    "apply_linear_to_cloud",
    "clip_cloud_by_polytope",
    "clip_cloud_by_polytope_gpu",
    "downsample_cloud",
    "set_gpu_unique",
]
...
class VSet:
    ...
    def __post_init__(self):
        if self.dtype not in ("float32", "float64"):
            raise ValueError(
                f"VSet dtype must be 'float32' or 'float64', got {self.dtype!r}"
            )
        V = np.asarray(self.V)
        if V.ndim == 1:
        ...
        if V.ndim != 2:
            raise ValueError(
                f"VSet.V must be a 2-D array, got shape {V.shape}"
            )
        if V.size == 0:
...
def box_corners(lb: np.ndarray, ub: np.ndarray, dtype="float32") -> np.ndarray:
    ...
    if lb.shape != ub.shape:
        raise ValueError(
            f"lb and ub must have the same shape, got {lb.shape} vs {ub.shape}"
        )
    if np.any(lb > ub):
        raise ValueError("lb must be element-wise <= ub")
    d = lb.size
