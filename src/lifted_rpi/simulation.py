...
__all__ = [
    ...
    "generate_trajectory",
    "simulate_trajectory_with_realistic_drag",
    "plot_trajectory_with_disturbance",
    "test_realistic_drag",
]
...
def eps_MRPI(A, W, epsilon, s_max=20):
    ...
    while s < s_max - 1:
        ...
        for i in range(I):
            ...
            if not st.success:
                _log.warning(
                    "Unsuccessful support evaluation h_W((A^%d)' * f_%d): %s",
                    s, i, st.message,
                )
            alpha_o[i] = h_W_i / g[i]
...
def plot_trajectory_with_disturbance(
    ...
    _log.info("Trajectory Type: %s", trajectory_type)
    _log.info("Simulation completed in %.2f seconds", total_time)
    _log.info("Control Rate: %.2f Hz", ns / total_time)
    _log.info("Average Position Error: %.4f m", np.mean(err))
    _log.info("Max Position Error: %.4f m", np.max(err))
    plt.show()
...
def test_realistic_drag(
    ...
    for traj_type in trajectory_types:
        _log.info("\nSimulating %s trajectory...", traj_type)
