...
__all__ = ["LiftedSetOpsGPU_NoHull", "hull_for_plot"]
...
class LiftedSetOpsGPU_NoHull:
    ...
    def __post_init__(self):
        ...
        if self.A.shape != (self.n, self.n):
            raise ValueError(
                f"A must be ({self.n}, {self.n}), got {self.A.shape}"
            )
        if self.B.shape != (self.n, self.m):
            raise ValueError(
                f"B must be ({self.n}, {self.m}), got {self.B.shape}"
            )
        if self.K.shape != (self.m, self.n):
            raise ValueError(
                f"K must be ({self.m}, {self.n}), got {self.K.shape}"
            )
    ...
    def _build_augmented_mats(self):
        ...
        if self.Edist is not None:
            ...
            if D_top.shape != (self.n, self.w):
                raise ValueError(
                    f"Edist must be ({self.n}, {self.w}), got {D_top.shape}"
                )
        else:
            if self.dist_rows is not None:
                if len(self.dist_rows) != self.w:
                    raise ValueError(
                        f"len(dist_rows)={len(self.dist_rows)} must equal w={self.w}"
                    )
                D_top = np.zeros((self.n, self.w), dtype=float)
                for j, r in enumerate(self.dist_rows):
                    if not (0 <= r < self.n):
                        raise ValueError(
                            f"dist_rows[{j}]={r} out of range [0, {self.n})"
                        )
                    D_top[r, j] = 1.0
            else:
                r = min(self.n, self.w)
    ...
    def _minkowski_gpu_batched_nohull(self, V1: np.ndarray, V2: np.ndarray):
        ...
        if self.compute_filter:
            ...
            if self.n_filter_passes > 1:
                _log.debug(
                    "Multi-pass filter (%d passes): %d candidates → %d kept",
                    self.n_filter_passes, total_candidates, filtered.shape[0],
                )
...
def hull_for_plot(vset: VSet) -> Tuple[np.ndarray, Optional["ConvexHull"]]:
    """
