...
__all__ = [
    ...
    "compute_closed_loop_cost",
    "compute_expected_stage_cost",
    "simulate_trajectory_cost",
    "plot_violations_and_cost",
...
def compute_violation_rate(
    ...
    if hasattr(G, "A") and hasattr(G, "b"):
        ...
        # Reconstruct pre-disturbance state (consistent with clip_with_graph)
        X_pre = X - W @ eng.D_top.T
        U = (eng.K @ X_pre.T).T + V

        lbW, ubW = G.bounds_fn(X_pre, U)
...
def fixed_point_reach(
    ...
    """
    _VALID_METRICS = {"hausdorff", "volume", "support_all", "support_state"}
    if convergence_metric not in _VALID_METRICS:
        raise ValueError(
            f"convergence_metric={convergence_metric!r} not in {sorted(_VALID_METRICS)}"
        )
    if max_iters < 1:
        raise ValueError(f"max_iters must be >= 1, got {max_iters}")
    if metric_tol < 0:
        raise ValueError(f"metric_tol must be >= 0, got {metric_tol}")
    ...
    for k in range(1, max_iters + 1):
        ...
        if verbose:
            ...
            _log.info(
                "[iter %03d] clip kept %d / %d points %s",
                k, Znext.nverts, base.nverts, timing_str,
            )
        ...
        if Znext.nverts == 0 and stop_if_empty:
            ...
            if verbose:
                _log.info("[iter %03d] Z became empty after clipping — stopping.", k)
        ...
        if track_facets:
            ...
            if verbose:
                _log.info("[iter %03d] facets: state=%d, all=%d", k, nf_state, nf_all)
        ...
        if track_violations:
            ...
            violation_rates.append(vr)
            if verbose:
                _log.info("[iter %03d] violation rate: %.4f (%d/%d)", k, vr, n_viol, n_tot)
        ...
        if track_cost and Q is not None and R is not None:
            ...
            costs.append(c)
            if verbose:
                _log.info("[iter %03d] %s cost: %.4f", k, cost_type, c)
        ...
        if convergence_metric == "volume":
            ...
            if verbose:
                _log.info(
                    "[iter %03d] volume(%s)=%.3e  drel=%.3e", k, volume_on, curr, mval,
                )
            if mval <= vol_eps:
                patience_count += 1
            else:
            ...
            if patience_count >= vol_pat:
                ...
                if verbose:
                    _log.info("%s.", termination_reason)
            ...
            metric_values.append(mval)
            if verbose:
                _log.info("[iter %03d] %s: gap=%.3e", k, convergence_metric, mval)
            if mval <= metric_tol:
                patience_count += 1
            else:
                patience_count = 0
            ...
            if verbose:
                ...
                _log.info(
                    "[iter %03d] hausdorff[%s]%s = %.3e", k, hausdorff_dims, normtxt, mval,
                )
            if mval <= metric_tol:
                patience_count += 1
            else:
