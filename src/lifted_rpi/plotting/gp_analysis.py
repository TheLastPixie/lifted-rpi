...
def extract_gp_results_for_visualization(
    ...
    print(f"Fitting GP model with {kernel_choice} kernel...")
...
def plot_acceleration_disturbance_spaces(
    ...
    print(f"  vx in [{vx.min():.3f}, {vx.max():.3f}], vy in [{vy.min():.3f}, {vy.max():.3f}]")
    print(f"  ux in [{ux.min():.3f}, {ux.max():.3f}], uy in [{uy.min():.3f}, {uy.max():.3f}]")
    print(f"  wx in [{wx.min():.3f}, {wx.max():.3f}], wy in [{wy.min():.3f}, {wy.max():.3f}]")
    ...
    print(f"  vx-wx: {corr_vx_wx:.3f},  ux-wx: {corr_ux_wx:.3f}")
    print(f"  vy-wy: {corr_vy_wy:.3f},  uy-wy: {corr_uy_wy:.3f}")
