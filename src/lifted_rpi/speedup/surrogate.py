...
def build_nystroem_surrogate(
    ...
    *,
    ...
    if verbose:
        ...
        _log.info(
            "[Nystroem-%d] built in %.3fs  RMSE mu=(%.6f,%.6f)  std=(%.6f,%.6f)",
            n_components, build_time, res_mu_x, res_mu_y, res_std_x, res_std_y,
        )
...
def build_poly_surrogate(
    ...
    *,
    ...
    if verbose:
        ...
        res_mu_y = np.sqrt(np.mean((gp_y.y_train_ - ridge_mu_y.predict(Phi)) ** 2))
        _log.info(
            "[Poly-%d] built in %.3fs  (%d features)  RMSE mu=(%.6f,%.6f)",
            degree, build_time, Phi.shape[1], res_mu_x, res_mu_y,
        )
